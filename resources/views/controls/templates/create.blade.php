...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('controls.templates.store') }}" method="POST">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required
                               placeholder="Ex: Contrôle mensuel sécurité"
                    ...
                    <div>
                        ...
                        <select name="review_type" required class="w-full rounded-lg border-gray-300 focus:border-red-500 focus:ring-red-500">
                            ...
                            <option value="{{ $value }}" {{ old('review_type') == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" 
                              placeholder="Décrivez l'objectif et le périmètre de ce template..."
                ...
                <div>
                    ...
                    <div id="checklistContainer" class="space-y-2">
                        @if(old('checklist'))
                            @foreach(old('checklist') as $index => $item)
                ...
                <div>
                    ...
                    <div id="attachmentsContainer" class="space-y-2">
                        @if(old('required_attachments'))
                            @foreach(old('required_attachments') as $index => $type)
                            <div class="flex gap-2">
                                <input type="text" name="required_attachments[]" value="{{ $type }}" 
                                       class="flex-1 rounded-lg border-gray-300 focus:border-red-500 focus:ring-red-500"
                                       placeholder="Ex: Capture d'écran, Log, Rapport PDF...">
                                <button type="button" onclick="this.parentElement.remove()" class="text-red-500 hover:text-red-700">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                    </svg>
