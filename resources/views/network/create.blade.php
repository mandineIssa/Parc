...
@extends('layouts.app')
@section('title', 'Ajouter une entrée réseau')
@section('content')
<div class="p-6 max-w-4xl mx-auto space-y-6">
    ...
    @endif
    <form method="POST" action="{{ route('network.store') }}" class="space-y-5">
        @csrf
        {{-- Type et Site --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="site" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach($sites as $s)
                            <option value="{{ $s }}" @selected(old('site') === $s)>{{ $s }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="type" required id="type-select" onchange="toggleSections()"
                        ...
                        <option value="">Sélectionner…</option>
                        <option value="plan_adressage"    @selected(old('type') === 'plan_adressage')>Plan d'adressage</option>
                        <option value="branchement_local" @selected(old('type') === 'branchement_local')>Branchement local</option>
                    </select>
                </div>
            </div>
        </div>
        {{-- Plan d'adressage --}}
        <div id="section-plan" class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm hidden">
            <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Plan d'adressage VLAN</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach(['vlan'=>'VLAN','adresse_reseau'=>'Adresse réseau','masque'=>'Masque sous-réseau','adresse_exclue'=>'Adresse exclue','adresse_dhcp'=>'Plage DHCP','default_gateway'=>'Default Gateway'] as $name => $label)
                <div>
        ...
        {{-- Branchement local --}}
        <div id="section-branchement" class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="equipement_reseau" value="{{ old('equipement_reseau') }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="equipement_connecte" value="{{ old('equipement_connecte') }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    ...
                    <select name="type_cable" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach($typesCable as $c)
                            <option value="{{ $c }}" @selected(old('type_cable') === $c)>{{ $c }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">IP équipement connecté</label>
                    <input type="text" name="adresse_ip_connecte" value="{{ old('adresse_ip_connecte') }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Commentaires</label>
                    <input type="text" name="commentaires" value="{{ old('commentaires') }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
        </div>
        <div class="flex justify-end gap-3">
            <a href="{{ route('network.index') }}" class="px-5 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition">Annuler</a>
            <button type="submit" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition">Enregistrer</button>
        </div>
    </form>
</div>
<script>
...
{{-- (même structure que create, avec @method('PUT') et valeurs pré-remplies) --}}
