...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    ...
    <form action="{{ route('parc.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="date" name="date_affectation" id="date_affectation" value="{{ old('date_affectation', date('Y-m-d')) }}" required
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            ...
            <div>
                ...
                <input type="date" name="date_retour_prevue" id="date_retour_prevue" value="{{ old('date_retour_prevue') }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        ...
        <div class="mb-4">
            ...
            <select name="statut_usage" id="statut_usage" required
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                <option value="en_service" {{ old('statut_usage') == 'en_service' ? 'selected' : '' }}>En service</option>
                <option value="reserve" {{ old('statut_usage') == 'reserve' ? 'selected' : '' }}>Réservé</option>
                <option value="maintenance" {{ old('statut_usage') == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
            </select>
            @error('statut_usage')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        ...
        <div class="mb-6">
            ...
            <textarea name="notes_affectation" id="notes_affectation" rows="3"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('notes_affectation') }}</textarea>
