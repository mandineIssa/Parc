    ...
    <form action="{{ route('maintenance.update', $maintenance->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        <div class="mb-4">
            ...
            <select name="numero_serie" id="numero_serie" required
                ...
                @foreach($equipments as $equipment)
                    <option value="{{ $equipment->numero_serie }}" {{ $maintenance->numero_serie == $equipment->numero_serie ? 'selected' : '' }}>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="date" name="date_depart" id="date_depart" 
                    value="{{ old('date_depart', $maintenance->date_depart) }}" required
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                @error('date_depart')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="date_retour_prevue" id="date_retour_prevue" 
                    value="{{ old('date_retour_prevue', $maintenance->date_retour_prevue) }}" required
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select name="type_maintenance" id="type_maintenance" required
                    ...
                    <option value="contractuelle" {{ $maintenance->type_maintenance == 'contractuelle' ? 'selected' : '' }}>Contractuelle</option>
                    <option value="autre" {{ $maintenance->type_maintenance == 'autre' ? 'selected' : '' }}>Autre</option>
                </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="prestataire" id="prestataire" 
                value="{{ old('prestataire', $maintenance->prestataire) }}" required
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div id="date_retour_reelle_div" class="{{ $maintenance->statut == 'terminee' ? '' : 'hidden' }}">
                ...
                <input type="date" name="date_retour_reelle" id="date_retour_reelle" 
                    value="{{ old('date_retour_reelle', $maintenance->date_retour_reelle) }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        ...
        <div class="mb-4">
            ...
            <textarea name="description_panne" id="description_panne" rows="3" required
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                placeholder="Décrivez la panne ou le besoin de maintenance...">{{ old('description_panne', $maintenance->description_panne) }}</textarea>
        ...
        <div class="mb-4">
            ...
            <textarea name="travaux_realises" id="travaux_realises" rows="3"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('travaux_realises', $maintenance->travaux_realises) }}</textarea>
        ...
        <div class="mb-6">
            ...
            <textarea name="observations" id="observations" rows="3"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('observations', $maintenance->observations) }}</textarea>
            @error('observations')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
...
<script>
// Show/hide date_retour_reelle field based on status
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (statut.value === 'terminee') {
        document.getElementById('date_retour_reelle_div').classList.remove('hidden');
    }
});
</script>
@endsection