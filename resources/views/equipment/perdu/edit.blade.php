...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Modifier la Déclaration de Perte</h1>
    
    <form action="{{ route('perdu.update', $perdu->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="numero_serie" id="numero_serie" required
                ...
                @foreach($equipments as $equipment)
                    <option value="{{ $equipment->numero_serie }}" {{ $perdu->numero_serie == $equipment->numero_serie ? 'selected' : '' }}>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="date" name="date_disparition" id="date_disparition" 
                    value="{{ old('date_disparition', $perdu->date_disparition->format('Y-m-d')) }}" required
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            ...
            <div>
                ...
                <select name="type_disparition" id="type_disparition" required
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    <option value="vol" {{ $perdu->type_disparition == 'vol' ? 'selected' : '' }}>Vol</option>
                    <option value="perte" {{ $perdu->type_disparition == 'perte' ? 'selected' : '' }}>Perte</option>
                    <option value="oubli" {{ $perdu->type_disparition == 'oubli' ? 'selected' : '' }}>Oubli</option>
                    <option value="destruction" {{ $perdu->type_disparition == 'destruction' ? 'selected' : '' }}>Destruction</option>
                </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="lieu_disparition" id="lieu_disparition" 
                value="{{ old('lieu_disparition', $perdu->lieu_disparition) }}" required
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        ...
        <div class="mb-4">
            ...
            <textarea name="circonstances" id="circonstances" rows="3" required
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('circonstances', $perdu->circonstances) }}</textarea>
            @error('circonstances')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <div id="plainte_fields" class="mt-2 {{ $perdu->plainte_deposee ? '' : 'hidden' }}">
                ...
                <input type="text" name="numero_plainte" id="numero_plainte" 
                    value="{{ old('numero_plainte', $perdu->numero_plainte) }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="number" step="0.01" name="valeur_assuree" id="valeur_assuree" 
                    value="{{ old('valeur_assuree', $perdu->valeur_assuree) }}"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            ...
            <div>
                ...
                <select name="statut_recherche" id="statut_recherche" required
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    <option value="en_cours" {{ $perdu->statut_recherche == 'en_cours' ? 'selected' : '' }}>En recherche</option>
                    <option value="trouve" {{ $perdu->statut_recherche == 'trouve' ? 'selected' : '' }}>Retrouvé</option>
                    <option value="definitif" {{ $perdu->statut_recherche == 'definitif' ? 'selected' : '' }}>Définitif</option>
                </select>
        ...
        <div class="mb-6">
            ...
            <textarea name="observations" id="observations" rows="3"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('observations', $perdu->observations) }}</textarea>
            @error('observations')
                <p class="text-red-500 text-xs italic">{{ $message }}</p>
            @enderror
        </div>
