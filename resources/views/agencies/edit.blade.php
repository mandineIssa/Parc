...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Modifier l'agence : {{ $agency->nom }}</h1>
        ...
        <form action="{{ route('agencies.update', $agency) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="adresse" id="adresse" rows="2"
                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('adresse', $agency->adresse) }}</textarea>
                @error('adresse')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('agencies.show', $agency) }}" class="text-gray-600 hover:text-gray-900">
                    Annuler
                </a>
            </div>
        </form>
