    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form method="POST" action="{{ route('users.store') }}" enctype="multipart/form-data" id="admin-user-create-form" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select id="departement" name="departement" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach($departementOptions ?? [] as $option)
                                <option value="{{ $option }}" {{ old('departement') === $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                        @error('departement')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select id="fonction" name="fonction" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach($posteOptions ?? [] as $option)
                                <option value="{{ $option }}" {{ old('fonction') === $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                        @error('fonction')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    @include('admin.users.partials.profil-rh-fields')
                    ...
                    @include('admin.users.partials.signature-fields', [
                        'formId' => 'admin-user-create-form',
                        'canvasId' => 'admin-user-create-sig-canvas',
                        'hiddenInputId' => 'admin_create_signature_canvas',
                    ])
