                ...
                <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="role" name="role" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                <option value="eod_n3" {{ old('role', $user->role) === 'eod_n3' ? 'selected' : '' }}>Signataire EOD N+3 (/eod/n3)</option>
                                <option value="eod_controller" {{ old('role', $user->role) === 'eod_controller' ? 'selected' : '' }}>Contrôleur EOD batch (/eod/controller)</option>
                        ...
                        <div>
                            ...
                            <select id="role_change" name="role_change" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                <option value="N1" {{ old('role_change', $user->role_change) === 'N1' ? 'selected' : '' }}>N+1 - Demandeur (Change)</option>
                                <option value="N2" {{ old('role_change', $user->role_change) === 'N2' ? 'selected' : '' }}>N+2 - Technicien (Change)</option>
                                <option value="N3" {{ old('role_change', $user->role_change) === 'N3' ? 'selected' : '' }}>N+3 - Validateur (Change + signature EOD /eod/n3)</option>
                                <option value="CONTROLLER" {{ old('role_change', $user->role_change) === 'CONTROLLER' ? 'selected' : '' }}>Controller — validation batch EOD (/eod/controller)</option>
                            </select>
                            <p class="mt-1 text-xs text-gray-500">Signature des fiches EOD côté N+3 : rôle <strong>N+3</strong>. Controller EOD : autre chemin <code class="text-xs">/eod/controller</code>.</p>
                            @error('role_change')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="rounded-lg border border-indigo-100 bg-indigo-50/50 p-4">
                        <label class="inline-flex items-start gap-3 cursor-pointer">
                            <input type="checkbox" name="eod_signature_only_ui" value="1" class="mt-1 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ old('eod_signature_only_ui', $user->eod_signature_only_ui) ? 'checked' : '' }}>
                            <span>
                                <span class="font-medium text-gray-800">Interface limitée au module EOD</span>
                                <span class="block text-xs text-gray-600 mt-1">Menu réduit : N+3 → <code class="text-xs">/eod/n3</code> uniquement ; Controller → <code class="text-xs">/eod/controller</code> uniquement.</span>
                            </span>
                        </label>
                    </div>
