...
<div class="mb-6">
    ...
    <!-- Accordéon : Filtres de Recherche -->
    <div class="card-cofina mb-4 overflow-hidden">
        <button onclick="toggleSection('section-filtres')" class="w-full flex items-center justify-between px-4 py-4 text-left focus:outline-none">
            <h2 class="text-xl font-bold text-cofina-red">🔍 Filtres de Recherche</h2>
            <svg id="icon-section-filtres" class="w-5 h-5 text-cofina-red transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
        </button>
        <div id="section-filtres" class="hidden px-4 pb-6">
    ...
    <div class="card-cofina mb-4 overflow-hidden">
        <button onclick="toggleSection('section-stats')" class="w-full flex items-center justify-between px-4 py-4 text-left focus:outline-none">
            <h2 class="text-xl font-bold text-cofina-red">📊 Statistiques</h2>
            <svg id="icon-section-stats" class="w-5 h-5 text-cofina-red transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
        </button>
        <div id="section-stats" class="hidden px-4 pb-6">
    ...
    <!-- Accordéon : Liste des Équipements -->
    <div class="card-cofina mb-4 overflow-hidden">
        <button onclick="toggleSection('section-liste')" class="w-full flex items-center justify-between px-4 py-4 text-left focus:outline-none">
            <h2 class="text-xl font-bold text-cofina-red">📋 Liste des Équipements</h2>
            <svg id="icon-section-liste" class="w-5 h-5 text-cofina-red transform transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
        </button>
        <div id="section-liste" class="hidden px-4 pb-6">
            <div class="mb-4">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-800">{{ $equipment->type }}</span>
                            </td>
    ...
    <!-- Accordéon : Filtres Actifs (ouvert automatiquement si filtres présents) -->
    @if(request()->hasAny(['type', 'etat', 'statut']))
    <div class="card-cofina mb-4 overflow-hidden">
        <button onclick="toggleSection('section-filtres-actifs')" class="w-full flex items-center justify-between px-4 py-4 text-left focus:outline-none">
            <h2 class="text-xl font-bold text-cofina-red">📌 Filtres Actifs</h2>
            <svg id="icon-section-filtres-actifs" class="w-5 h-5 text-cofina-red transform transition-transform duration-300 rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
        </button>
        <div id="section-filtres-actifs" class="px-4 pb-6">{{-- ouvert par défaut si filtres actifs --}}
...
<script>
    ...
    // Ouvrir automatiquement la liste si des filtres sont actifs
    @if(request()->hasAny(['type', 'etat', 'statut']))
        document.addEventListener('DOMContentLoaded', function () {
            toggleSection('section-liste');
            toggleSection('section-stats');
        });
    @endif
</script>
@endsection