...
class DecelerElectroniqueDashboardController extends Controller
{
    /**
     * Display the deceler electronique dashboard.
     */
    public function index()
    {
        // Récupérer tous les stocks de type electronique avec type_stock = 'deceler'
        $stocks = Stock::with(['equipment', 'deceler'])
            ->whereHas('equipment', function($query) {
                $query->where('type', 'electronique');
            })
        ...
        $categorie_ids = Equipment::where('type', 'electronique')
            ->select('categorie_id')
            ->distinct()
            ->pluck('categorie_id');
        ...
        return view('dashboard.deceler-electronique', compact('stocks', 'stats', 'categoryStats'));
    }
    ...
    public function export(Request $request)
    {
        ...
        $filename = 'deceler_electronique_' . date('Y-m-d') . '.csv';
        $headers = [
    ...
    public function show($id)
    {
        ...
        // Vérifier que c'est bien un stock deceler electronique
        if ($stock->type_stock !== 'deceler' || !$stock->equipment || $stock->equipment->type !== 'electronique') {
            abort(404);
        }
