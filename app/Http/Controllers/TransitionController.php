...
/**
 * Soumettre une transition simple (maintenance, hors service, etc.)
 */
public function submitSimpleTransition(Request $request, Equipment $equipment)
{
    DB::beginTransaction();
    ...
    try {
        ...
        $transitionType = $request->input('transition_type');

        // Validation selon le type de transition
        if ($transitionType === 'parc_to_maintenance') {
            $validated = $request->validate([
                'transition_type' => 'required|string',
            ...
            // Si Super Admin, approuver directement
            if ($isSuperAdmin) {
                $this->processParcToMaintenance($equipment, $data, $approval);
                $approval->update([
                    'status' => 'approved',
                    'approved_by' => $user->id,
                    'approved_at' => now(),
        ...
        // Autres types de transitions à implémenter...
        throw new \Exception("Type de transition non supporté: $transitionType");
        ...
        DB::rollBack();
        Log::error('Erreur submitSimpleTransition: ' . $e->getMessage());
        return response()->json([
            'success' => false,
            'message' => 'Erreur: ' . $e->getMessage(),
        ], 500);
...
/**
 * Traiter la transition Parc → Maintenance
 */
private function processParcToMaintenance(Equipment $equipment, array $data, TransitionApproval $approval)
{
    // Mettre à jour l'équipement
    $equipment->update([
        'statut' => 'maintenance',
    ...
    if ($equipment->parc) {
        $equipment->parc->update([
            'statut_usage' => 'en_maintenance',
            'notes_affectation' => 'En maintenance - ' . ($data['description_probleme'] ?? ''),
        ]);
    }
}
}
